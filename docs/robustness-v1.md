...
- **Calibration finding (v2 recalibration item, NOT rating tuning).** All models show intercept a ~ +0.5 and slope b < 1 (overconfident). A radiant-side effect is only a HYPOTHESIS and cannot be the sole cause: radiant win rate ~53% => logit ~ +0.12, far short of +0.5. Side-neutralization mainly touches the intercept, not the slope. The actual fix is a strictly-rolling **OOS Platt** layer (intercept + slope); measured in `calibration-v1.md`. Neither changes the ranking; both are applied before any probability ships.
